...
fun Application.configureRouting() {
    ...
    if (rconPassword.isNullOrBlank()) {
        environment.log.warn("RCON password not set in environment (MC_RCON_PASSWORD). /give-map will fail.")
    }
    ...
    routing {
        get("/history/{sessionId}") {
            val sessionId = call.parameters["sessionId"] ?: return@get call.respond(
                HttpStatusCode.BadRequest, "Missing session ID"
            )
        ...
        post("/upload-map") {
            try {
                ...
                call.receiveMultipart().forEachPart { part ->
                    when (part) {
                        ...
                        is PartData.FormItem -> {
                            when (part.name) {
                                "grid_x" -> gridX = part.value.toIntOrNull() ?: 1
                                "grid_y" -> gridY = part.value.toIntOrNull() ?: 1
                                "session_id" -> sessionId = part.value
                            }
                        }
                        else -> {}
                    }
                ...
                call.respond(
                    ...
                    UploadResponse(status = "error", message = "Backend crashed: ${e.localizedMessage ?: "Unknown error"}")
                )
            }
        }
        ...
        post("/give-map") {
            try {
                ...
                call.respond(HttpStatusCode.OK, GiveMapResponse(status = "success"))

            } catch (e: Exception) {
                call.application.log.error("RCON failed: ", e)
                call.respond(
                    HttpStatusCode.InternalServerError,
                    GiveMapResponse(status = "error", message = "RCON failed: ${e.localizedMessage ?: "Unknown error"}")
                )
            }
        }
